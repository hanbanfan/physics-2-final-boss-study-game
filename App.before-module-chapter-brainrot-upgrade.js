...
const COURSE_SCHEDULE = [
  {
    module: "Module 1",
    dates: "6/08 - 6/24",
    focus: "Waves, electric charge, electric field, and Gauss's Law",
    chapters: "Ch. 16, 22, 23, 24",
  ...
  },
  {
    ...
    chapters: "Ch. 29, 30, 31",
    exam: "Module 3 Exam on 7/20",
  },
  {
    module: "Module 4",
    dates: "7/22 - 8/02",
    focus: "Final review and remaining final topics",
    chapters: "Ch. 33, 32 / Final Review",
    exam: "Final Exam due 8/02 at 11:59 PM",
  },
];

const FLASHCARD_EXTRAS = {
  "16": [
    ["When do I use v = fλ?", "When the problem gives wave speed, frequency, or wavelength."],
    ["When do I use f = 1/T?", "When the problem gives period or asks for period/frequency."],
    ["When do I use T₂ = T₁(v₂/v₁)²?", "When the same string has two different wave speeds/tensions."],
    ["Why is tension squared in the string shortcut?", "Because v = √(T/μ), so T is proportional to v²."],
    ["When do I use phase math?", "When the problem gives wavelength, phase, and positions like r = 8.0 m."],
    ["What is k?", "Wave number: k = 2π/λ."],
    ["What should a phase answer use as units?", "Radians."],
    ["What is the first move on any wave problem?", "Label givens: v, f, λ, T, tension, phase, or r."],
  ],
  "22": [
    ["Coulomb clue words?", "Charge, q₁, q₂, separated, distance, attract, repel, electric force."],
    ["What should I do with microcoulombs?", "Convert μC to C before using Coulomb's Law."],
    ["What part gets squared?", "The distance r."],
    ["If one charge is negative, is the force magnitude negative?", "No. Magnitude is positive; sign tells direction."],
    ["What is the direction rule?", "Same signs repel; opposite signs attract."],
  ],
  "23": [
    ["Electric field clue words?", "Electric field, force per charge, N/C, test charge."],
    ["How do I find force from field?", "F = qE."],
    ["How do I find charge from field and force?", "q = F/E."],
    ["What happens to force if charge doubles?", "Force doubles if E stays the same."],
    ["Direction for negative charge?", "Opposite the field."],
  ],
  "24": [
    ["Flux clue words?", "Flux, surface, area, angle, Gaussian surface, enclosed charge."],
    ["When do I use Φ = EAcosθ?", "When E, A, and angle are given."],
    ["When do I use Φ = q_enc/ε₀?", "When closed surface or enclosed charge is given."],
    ["What does θ = 90° do to flux?", "Makes flux zero."],
    ["What does θ = 0° do to flux?", "Makes flux maximum."],
  ],
  "25": [
    ["Potential clue words?", "Voltage, volts, electric potential, potential energy, energy per charge."],
    ["What does voltage mean?", "Energy per charge."],
    ["How do I find energy from voltage?", "U = qV."],
    ["How do I find charge from energy and voltage?", "q = U/V."],
    ["Is voltage a vector?", "No, it is scalar."],
  ],
  "26": [
    ["Capacitance clue words?", "Capacitor, plates, charge stored, farad, dielectric."],
    ["What does C measure?", "How much charge is stored per volt."],
    ["How do I find Q?", "Q = CΔV."],
    ["How do I find ΔV?", "ΔV = Q/C."],
    ["What does a dielectric usually do?", "Increases capacitance."],
  ],
  "27": [
    ["Ohm's Law clue words?", "Current, voltage, resistance, resistor, ohm, amp, battery."],
    ["How do I find current?", "I = V/R."],
    ["How do I find voltage?", "V = IR."],
    ["How do I find resistance?", "R = V/I."],
    ["What happens to current if resistance increases?", "Current decreases if voltage stays the same."],
  ],
  "28": [
    ["Circuit clue words?", "Series, parallel, junction, loop, power, watts, resistor network."],
    ["What is same in series?", "Current."],
    ["What is same in parallel?", "Voltage."],
    ["How do series resistors combine?", "Add directly."],
    ["How do parallel resistors combine?", "Use reciprocals."],
    ["What does Kirchhoff's junction rule conserve?", "Current/charge."],
    ["What does Kirchhoff's loop rule conserve?", "Energy/voltage."],
  ],
  "29": [
    ["Magnetic force clue words?", "Moving charge, magnetic field, tesla, angle, velocity."],
    ["Why does angle matter?", "Only the perpendicular part of velocity counts."],
    ["What formula uses sinθ?", "F = qvBsinθ."],
    ["When is magnetic force zero?", "v = 0 or θ = 0°/180°."],
    ["When is magnetic force maximum?", "θ = 90°."],
  ],
  "30": [
    ["Induction clue words?", "Changing flux, coil, turns, induced emf, Faraday, Lenz."],
    ["What must change for induction?", "Magnetic flux."],
    ["How do I find emf magnitude?", "|ε| = NΔΦB/Δt."],
    ["What does more turns do?", "Increases emf."],
    ["What does Lenz's Law tell me?", "Direction opposes the change."],
  ],
  "31": [
    ["EM wave clue words?", "Light, electromagnetic wave, vacuum, speed of light, frequency, wavelength."],
    ["What speed do I use for light?", "c = 3.00 × 10⁸ m/s."],
    ["How do I find wavelength?", "λ = c/f."],
    ["How do I find frequency?", "f = c/λ."],
    ["High frequency means what?", "Short wavelength."],
  ],
  "LOG": [
    ["What does log mean?", "Common log, base 10."],
    ["What does ln mean?", "Natural log, base e."],
    ["What do logs undo?", "Exponents."],
    ["How do I solve log(A)=B?", "Rewrite as A = 10^B."],
    ["How do I solve ln(A)=B?", "Rewrite as A = e^B."],
    ["How do I bring down an exponent?", "Use log(a^x)=xlog(a)."],
  ],
};

function getEnhancedCards(chapter) {
  const extras = FLASHCARD_EXTRAS[String(chapter.number)] || [];
  const combined = [...chapter.cards, ...extras];
  const seen = new Set();
...
function getStudyBlueprint(chapter) {
  const number = String(chapter.number);

  const blueprintByChapter = {
    "16": {
      quizMission: "Identify which wave relationship is being tested: basic wave speed, period/frequency, string tension, or phase.",
      mustKnow: "v = fλ, f = 1/T, v = √(T/μ), T₂ = T₁(v₂/v₁)², k = 2π/λ.",
      testMove: "Label every number as v, f, λ, T, tension, r, phase, or k before solving.",
    },
    "22": {
      quizMission: "Use Coulomb's Law and direction rules for charges.",
      mustKnow: "F = kq₁q₂/r², k = 9 × 10⁹, same repel, opposite attract.",
      testMove: "Convert charge units if needed, square r, then decide direction.",
    },
    "23": {
      quizMission: "Separate electric field from electric force.",
      mustKnow: "E = F/q and F = qE.",
      testMove: "Ask: am I solving for field, force, or charge?",
    },
    "24": {
      quizMission: "Recognize flux through a surface versus total flux through a closed surface.",
      mustKnow: "ΦE = EAcosθ and ΦE = q_enc/ε₀.",
      testMove: "If angle/area appear, use EAcosθ. If enclosed charge appears, use Gauss.",
    },
    "25": {
      quizMission: "Treat voltage as energy per charge.",
      mustKnow: "V = U/q and U = qV.",
      testMove: "Label U, q, and V before solving.",
    },
    "26": {
      quizMission: "Use capacitance as charge stored per voltage difference.",
      mustKnow: "C = Q/ΔV, Q = CΔV, ΔV = Q/C.",
      testMove: "Decide whether the unknown is capacitance, charge, or voltage.",
    },
    "27": {
      quizMission: "Rearrange Ohm's Law fast and correctly.",
      mustKnow: "V = IR, I = V/R, R = V/I.",
      testMove: "Cover the unknown in V = IR to pick the right rearrangement.",
    },
    "28": {
      quizMission: "Identify series vs parallel and apply circuit conservation rules.",
      mustKnow: "Series: same current. Parallel: same voltage. P = IV.",
      testMove: "Before calculating, write SERIES or PARALLEL at the top.",
    },
    "29": {
      quizMission: "Use magnetic force only when charge is moving through a magnetic field.",
      mustKnow: "F = qvBsinθ.",
      testMove: "Check v and θ. If v = 0 or θ = 0°, force is zero.",
    },
    "30": {
      quizMission: "Recognize that changing magnetic flux creates induced emf.",
      mustKnow: "ε = -NΔΦB/Δt.",
      testMove: "Ask: what is changing — B, area, angle, or time?",
    },
    "31": {
      quizMission: "Use the light-wave relationship with c.",
      mustKnow: "c = fλ, c = 3.00 × 10⁸ m/s.",
      testMove: "Do not use sound speed for light.",
    },
    "LOG": {
      quizMission: "Use logs to undo exponents and exponentials to undo logs.",
      mustKnow: "log = base 10, ln = base e, log(A)=B → A=10^B.",
      testMove: "Isolate the log or exponential part first.",
    },
  };

  return blueprintByChapter[number] || {
    quizMission: "Identify the topic, label variables, and pick the formula.",
    mustKnow: chapter.formula,
    testMove: "Use GIVEN → MATH → SOLUTION.",
  };
}

function getGameChoices(correct, cards, index) {
  const answers = cards.map((card) => card[1]).filter((answer) => answer !== correct);
  const unique = [...new Set(answers)];
  const choices = [correct];

  let i = index;
  while (choices.length < 4 && unique.length > 0) {
    const candidate = unique[i % unique.length];
    if (!choices.includes(candidate)) choices.push(candidate);
    i += 1;
  }

  while (choices.length < 4) {
    choices.push("Not this one");
  }
...
export default function App() {
  ...
  const enhancedCards = getEnhancedCards(chapter);
  const clip = chapter.clips[clipIndex];
  const card = enhancedCards[flashIndex % enhancedCards.length];
  const gameCard = enhancedCards[gameIndex % enhancedCards.length];
  const gameChoices = getGameChoices(gameCard[1], enhancedCards, gameIndex);
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Button label="Schedule" type="gold" onPress={() => setScreen("schedule")} />
          ...
          <Button label="Chapter Game" type="purple" onPress={() => { setGameIndex(0); setGameScore(0); setGameMessage(""); setScreen("game"); }} />
  ...
  if (screen === "study") {
    const blueprint = getStudyBlueprint(chapter);
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Box title="Quiz Mission">
            {blueprint.quizMission}
          </Box>

          <Box title="Must Know Before the Quiz">
            {blueprint.mustKnow}
          </Box>
          ...
          <Button label="Play Chapter Game" type="gold" onPress={() => { setGameIndex(0); setGameScore(0); setGameMessage(""); setScreen("game"); }} />
          <Button label="Back Home" type="secondary" onPress={() => setScreen("home")} />
        </View>
      </ScrollView>
    );
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          <Text style={styles.sectionTitle}>
            Card {flashIndex + 1} of {enhancedCards.length}
          </Text>
          ...
          <Button
            ...
            type="purple"
            onPress={() => {
              setFlashIndex(flashIndex + 1 < enhancedCards.length ? flashIndex + 1 : 0);
              setShowAnswer(false);
            }}
          />
  ...
  if (screen === "schedule") {
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <Text style={styles.subtitle}>Use this as your quiz/exam battle map.</Text>

        <View style={styles.card}>
          {COURSE_SCHEDULE.map((item) => (
            <View key={item.module} style={styles.scheduleCard}>
              <Text style={styles.scheduleModule}>{item.module}</Text>
              <Text style={styles.scheduleDates}>{item.dates}</Text>
              <Text style={styles.scheduleText}>Chapters: {item.chapters}</Text>
              <Text style={styles.scheduleText}>Focus: {item.focus}</Text>
              <Text style={styles.scheduleExam}>{item.exam}</Text>
            </View>
          ))}

          <Box title="How to Use This Schedule">
            {"1. Study the current module first.\\n2. Use Study Guide before lecture/quiz.\\n3. Use Flashcards for recall.\\n4. Use Chapter Game to test yourself.\\n5. Use Homework Solver for actual problems."}
          </Box>
  ...
  if (screen === "game") {
    return (
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>Chapter Game</Text>
        <Text style={styles.subtitle}>Chapter {chapter.number}: {chapter.title}</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.gameQuestionBox}>
            <Text style={styles.gameLabel}>QUESTION {gameIndex + 1}</Text>
            <Text style={styles.gameQuestion}>{gameCard[0]}</Text>
          </View>
          ...
          {gameChoices.map((choice) => (
            <Pressable
              ...
              onPress={() => {
                if (choice === gameCard[1]) {
                  setGameScore((old) => old + 10);
                  setGameMessage("Correct. That quiz demon has been handled.");
                } else {
                  setGameMessage(`Not quite. Correct answer: ${gameCard[1]}`);
                }
              }}
            >
          ...
          <Button
            ...
            type="gold"
            onPress={() => {
              setGameIndex(gameIndex + 1 < enhancedCards.length ? gameIndex + 1 : 0);
              setGameMessage("");
            }}
          />
