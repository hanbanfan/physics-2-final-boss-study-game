...
const FORMULA_MAP = [
  ["Waves", "v = f lambda", "frequency, wavelength, period, sound, light", "Connects wave speed, frequency, and wavelength."],
  ["Coulomb's Law", "F = k q1 q2 / r^2", "q1, q2, distance, force", "Finds force between two charges."],
  ["Electric Field", "E = F / q", "force, charge, N/C", "Defines field as force per charge."],
  ["Electric Flux", "PhiE = E A cos(theta)", "flux, area, angle, surface", "Counts electric field through a surface."],
  ["Gauss's Law", "PhiE = q enclosed / epsilon0", "closed surface, symmetry, enclosed charge", "Connects closed-surface flux to enclosed charge."],
  ["Electric Potential", "V = U / q", "voltage, energy, charge", "Defines voltage as energy per charge."],
  ["Capacitance", "C = Q / delta V", "capacitor, charge, voltage", "Charge stored per volt."],
  ...
  ["Magnetic Force", "F = q v B sin(theta)", "charge, velocity, B field, angle", "Force on a moving charge in a magnetic field."],
  ["Faraday's Law", "emf = -N delta PhiB / delta t", "changing flux, coil, Faraday, Lenz", "Changing magnetic flux creates emf."],
];

const CALC3_TOOLS = [
  ["Vectors", "A = <Ax, Ay, Az>", "Fields, forces, velocity, and displacement have direction. Treat them like arrows with components."],
  ["Magnitude", "|A| = sqrt(Ax^2 + Ay^2 + Az^2)", "Magnitude is vector size. Use it when the answer asks for strength of a field or force."],
  ["Dot product", "A dot B = A B cos(theta)", "Dot products measure how much vectors line up. Flux and work use this idea."],
  ["Cross product", "|A cross B| = A B sin(theta)", "Cross products measure perpendicular interaction. Magnetic force uses this idea."],
  ["Gradient", "E = -grad V", "Electric field points downhill from electric potential. Steeper voltage change means stronger field."],
  ["Divergence", "div E", "Divergence measures field spreading outward. Gauss's Law is divergence thinking."],
  ["Curl", "curl E", "Curl measures field swirl. Induction uses curl thinking."],
  ["Surface integral", "double integral E dot dA", "Flux adds up field through a surface."],
  ["Line integral", "integral E dot dr", "Voltage difference and emf add up fields along a path."],
];
...
const DERIVATIONS = [
  {
    ...
    steps: ["Speed = distance / time", "Distance for one cycle = lambda", "Time for one cycle = T", "v = lambda / T", "f = 1 / T", "Therefore v = f lambda"],
  },
  {
  ...
  },
  {
    ...
    steps: ["More charge means more force", "Force depends on q1 q2", "Field spreads in 3D", "Sphere area grows like r^2", "Force drops like 1 / r^2", "Add constant k"],
  },
  {
    title: "Flux",
    formula: "PhiE = E A cos(theta)",
    simple: "Flux counts field through area.",
    steps: ["Only perpendicular field counts", "Perpendicular part is E cos(theta)", "Multiply by area A", "PhiE = E A cos(theta)"],
  },
  {
    title: "Gauss's Law",
    formula: "PhiE = q enclosed / epsilon0",
    simple: "Closed-surface flux depends on charge inside.",
    steps: ["Put charge inside a sphere", "Field spreads evenly", "Flux = E times area", "Area = 4 pi r^2", "r^2 cancels", "Result is q enclosed / epsilon0"],
  },
  {
    title: "Ohm's Law",
    formula: "V = I R",
  ...
  },
  {
    ...
    steps: ["More charge means more force", "More speed means more force", "More B means more force", "Only perpendicular motion counts", "That gives sin(theta)"],
  },
  {
    title: "Faraday's Law",
    formula: "emf = -N delta PhiB / delta t",
...
const CHAPTERS = [
  {
    ...
    math: ["v = f lambda", "f = 1 / T", "lambda = v / f", "T = 1 / f"],
    lesson: [
      ...
      ["Formula", "Use v = f lambda when speed, frequency, and wavelength appear."],
      ["Doppler", "Motion changes observed frequency."],
    ],
    example: ["Wave speed", "frequency, wavelength", "f = 10 Hz, lambda = 2 m", "v", "v = f lambda", "v = 10 x 2", "20 m/s"],
    cards: [
      ...
      ["When do you use v = f lambda?", "When speed, frequency, and wavelength are connected."],
      ["What is Doppler effect?", "Observed frequency changes because source or observer moves."],
    ],
    quiz: ["A wave has f = 5 Hz and lambda = 3 m. What formula solves speed?", ["v = f lambda", "V = I R", "C = Q / delta V"], "v = f lambda"],
  },
  {
    number: 22,
    moduleId: 1,
    ...
    homeworkSkills: [
      ...
      "Handle r^2 correctly.",
      "Understand conductors versus insulators.",
      "Use scientific notation with k = 9 x 10^9.",
    ],
    teacherTests: "Can you identify q1, q2, and r, then calculate force without forgetting that distance is squared?",
    ...
    math: ["F = k q1 q2 / r^2", "k = 9 x 10^9", "r^2 means distance squared"],
    lesson: [
      ...
      ["Formula", "Use F = k q1 q2 / r^2."],
    ],
    example: ["Electric force", "q1, q2, r", "q1 = 2 C, q2 = 3 C, r = 1 m", "F", "F = k q1 q2 / r^2", "F = (9x10^9)(2)(3)/(1^2)", "5.4 x 10^10 N"],
    cards: [
      ...
      ["Why does r^2 matter?", "Force gets weaker very quickly as distance increases."],
      ["What are the Coulomb clues?", "q1, q2, r, force, attract, repel."],
    ],
  ...
  },
  {
    ...
    homeworkSkills: [
      ...
      "Use angle correctly in E A cos(theta).",
    ...
    math: ["PhiE = E A cos(theta)", "Gauss: total flux = q enclosed / epsilon0", "cos 0 = 1", "cos 90 = 0"],
    lesson: [
    ...
    example: ["Electric flux", "E, area, angle", "E = 10 N/C, A = 2 m^2, theta = 0", "PhiE", "PhiE = E A cos(theta)", "PhiE = 10 x 2 x 1", "20 N m^2/C"],
    cards: [
      ["What is flux?", "Field passing through area."],
      ["What formula gives simple flux?", "PhiE = E A cos(theta)."],
      ["When is flux maximum?", "theta = 0 degrees."],
      ["When is flux zero?", "theta = 90 degrees."],
  ...
  },
  {
    ...
    traps: [
      ...
      "Use delta V for voltage difference.",
    ],
    math: ["C = Q / delta V", "Q = C delta V", "delta V = Q / C"],
    lesson: [
      ...
      ["Formula", "Use C = Q / delta V."],
    ],
    example: ["Capacitance", "charge, voltage", "Q = 6 C, delta V = 3 V", "C", "C = Q / delta V", "C = 6 / 3", "2 F"],
    cards: [
      ["What does a capacitor store?", "Charge and energy."],
      ["What is capacitance?", "Charge stored per volt."],
      ["What formula defines capacitance?", "C = Q / delta V."],
  ...
  },
  {
    ...
    traps: [
      ...
      "Use amps for current and ohms for resistance.",
    ],
    ...
    example: ["Current", "voltage, resistance", "V = 12 V, R = 4 ohms", "I", "I = V / R", "I = 12 / 4", "3 A"],
    cards: [
  ...
  },
  {
    ...
    traps: [
      ...
      "Do not forget units: watts, amps, volts, ohms.",
    ],
    ...
    example: ["Power", "current, voltage", "I = 3 A, V = 12 V", "P", "P = I V", "P = 3 x 12", "36 W"],
    cards: [
  ...
  },
  {
    ...
    teacherTests: "Can you identify q, v, B, and theta and know whether magnetic force exists?",
    ...
    traps: [
      ...
      "sin theta controls the angle part.",
    ],
    math: ["F = q v B sin(theta)", "sin 90 = 1", "sin 0 = 0"],
    lesson: [
      ...
      ["Formula", "Use F = q v B sin(theta)."],
    ],
    example: ["Magnetic force", "q, v, B, angle", "q = 2 C, v = 3 m/s, B = 4 T, theta = 90", "F", "F = q v B sin(theta)", "F = 2 x 3 x 4 x 1", "24 N"],
    cards: [
      ...
      ["What formula gives magnetic force magnitude?", "F = q v B sin(theta)."],
  ...
  },
  {
    ...
    homeworkSkills: [
      ...
      "Use emf = N delta Phi / delta t for magnitude.",
    ...
    math: ["emf magnitude = N delta Phi / delta t", "Negative sign = Lenz's Law", "More turns = more emf"],
    lesson: [
      ["Flux", "Magnetic flux is B field through area."],
      ["Change", "Changing flux creates emf."],
      ["Lenz", "The induced effect opposes the change."],
      ["Formula", "Use emf = -N delta PhiB / delta t."],
    ],
    example: ["Induced emf", "turns, flux, time", "N = 10, delta Phi = 0.20 Wb, delta t = 0.50 s", "emf", "emf = N delta Phi / delta t", "emf = 10(0.20)/0.50", "4 V"],
    cards: [
  ...
  },
  {
    ...
    math: ["c = f lambda", "c = 3 x 10^8 m/s", "lambda = c / f", "f = c / lambda"],
    lesson: [
      ["Fields", "Electric and magnetic fields travel together."],
      ["Vacuum", "EM waves do not need air."],
      ["Light", "Light is an EM wave."],
      ["Formula", "Use c = f lambda."],
    ],
    example: ["Wavelength of light", "frequency, speed of light", "f = 6 x 10^14 Hz, c = 3 x 10^8 m/s", "lambda", "lambda = c / f", "lambda = (3 x 10^8)/(6 x 10^14)", "5 x 10^-7 m"],
    cards: [
      ["What is light?", "An electromagnetic wave."],
      ["Can EM waves travel in vacuum?", "Yes."],
      ["What is c?", "Speed of light."],
      ["What is the EM wave equation?", "c = f lambda."],
