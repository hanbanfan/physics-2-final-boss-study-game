...
function solveElectrostaticsHomework(text) {
  ...
  const lower = lowerRaw
    ...
    .replaceAll("superscript minus", "e-")
    .replaceAll("superscript negative", "e-")
    .replaceAll("superscript", "e")
    .replaceAll("times 10", "e")
  ...
  function sciValue(patterns) {
    // Handles MasteringPhysics text like:
    // 2.0 times 10 Superscript 10
    // 7.25 times 10 Superscript negative 11
    // 1.46 * 10 Superscript minus 6
    const mastering = lowerRaw.match(
      /([-+]?\d*\.?\d+)\s*(?:times|\*|x)\s*10\s*superscript\s*(?:(minus|negative)\s*)?([-+]?\d+)/i
    );

    if (mastering) {
      const base = Number(mastering[1]);
      const sign = mastering[2] ? -1 : 1;
      const exp = sign * Number(mastering[3]);
      if (Number.isFinite(base) && Number.isFinite(exp)) {
        return base * Math.pow(10, exp);
      }
    }

    const caret = lowerRaw.match(
      /([-+]?\d*\.?\d+)\s*(?:times|\*|x)\s*10\s*(?:\^|\*\*)\s*([-+]?\d+)/i
    );

    if (caret) {
      const base = Number(caret[1]);
      const exp = Number(caret[2]);
      if (Number.isFinite(base) && Number.isFinite(exp)) {
        return base * Math.pow(10, exp);
      }
    }
    ...
    for (const pattern of patterns) {
      ...
      if (m) {
        ...
        if (Number.isFinite(base) && Number.isFinite(exp)) return base * Math.pow(10, exp);
      }
    }
    return null;
  }

  function simpleNumberBefore(unitPattern) {
    const m = lower.match(new RegExp("([-+]?\\d*\\.?\\d+)\\s*" + unitPattern, "i"));
    return m ? Number(m[1]) : null;
  }

  function allSimpleBefore(unitPattern) {
  ...
  // ------------------------------------------------------------
  // Charge from excess electrons: Q = -Ne
  // ------------------------------------------------------------
  if (hasAny(["electrons are added", "excess electrons", "number of electrons"])) {
    const n =
      sciValue([
        /([-+]?\d*\.?\d+)\s*(?:x|\*)?\s*e\s*([-+]?\d+)\s*electrons/,
        /([-+]?\d*\.?\d+)\s*(?:x|\*)?\s*10\s*\^?\s*([-+]?\d+)\s*electrons/,
      ]) ||
      simpleNumberBefore("electrons");
    ...
    if (n) {
      ...
      solution =
        ...
        `Answer: ${Q.toExponential(2)} C`;
    }

    return makeElectroResult({
      topic: "Ch. 23/24 HW: Charge from Electrons",
      unknown: "Net charge Q",
      formula: "Q = N(-e)",
      math: "Multiply the number of added electrons by the charge of one electron.",
      solution,
      trap: "Adding electrons makes the charge negative. Removing electrons makes it positive.",
      variableMap: "N = number of electrons\ne = 1.60 x 10^-19 C\nQ = net charge",
    });
  }

  // ------------------------------------------------------------
  // Conductor/insulator/polarization conceptual questions
  // ------------------------------------------------------------
  if (hasAny(["plastic balls", "copper ball", "test charge", "weakly attracted", "strongly attracted", "strongly repelled"])) {
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Conductors, Insulators, and Polarization",
      unknown: "Attractive, repulsive, or neither",
      formula: "Like charges repel; opposite charges attract; neutral objects can attract by polarization.",
      math:
        "Use the positive test charge to identify each ball:\n\n" +
        "Strong repulsion from positive test charge -> object is positive.\n" +
        "Strong attraction to positive test charge -> object is negative OR neutral conductor very close.\n" +
        "Weak attraction -> neutral insulator polarization.\n" +
        "Neutral conductor near charge -> strong attraction by induced charge separation.\n\n" +
        "Then compare the two objects in the question.",
      solution:
        "From your homework pattern:\n" +
        "Ball B strongly repels positive T -> B is +Q.\n" +
        "Ball A strongly attracts positive T and is plastic -> A is -Q.\n" +
        "Ball C weakly attracts positive T -> C is neutral plastic.\n" +
        "Ball D is uncharged copper -> neutral conductor.\n\n" +
        "A-B: attractive.\n" +
        "A-C: attractive, smaller magnitude than A-B.\n" +
        "A-D: attractive.\n" +
      ...
      variableMap: "A = negative plastic\nB = positive plastic\nC = neutral plastic insulator\nD = neutral copper conductor\nT = positive test charge",
    });
  }

  if (hasAny(["small metal ball", "negative charge", "end a", "end b", "rod", "great many contacts", "several contacts"])) {
    const conductor = hasAny(["conducting rod", "conductor", "charges have already freely separated", "spread evenly"]);
    const insulator = hasAny(["plastic rod", "insulator"]);

    return makeElectroResult({
      topic: "Ch. 23/24 HW: Charging Rods by Induction and Contact",
      unknown: "Attracted, repelled, or charge arrangement",
      formula: "Conductor: charge spreads. Insulator: charge stays local.",
      math:
        "First approach to a neutral object:\n" +
        "A charged ball polarizes the rod, so the near end is attracted.\n\n" +
        "After contact:\n" +
        "Electrons transfer if the ball is negative.\n\n" +
        "If rod is an insulator:\n" +
        "Negative charge stays near end A.\n\n" +
        "If rod is a conductor:\n" +
        "Negative charge spreads across the rod.",
      solution:
        "For an insulating/plastic rod:\n" +
        "First approach to neutral end A -> weakly attracted.\n" +
        "After several negative contacts -> negative charge on end A, end B almost neutral.\n" +
        "Approach end A again -> strongly repelled.\n\n" +
        "For a conducting rod:\n" +
        "First approach to neutral end A -> attracted.\n" +
        "After many contacts -> negative charge spreads evenly on both ends.\n" +
        "Approach end A or B again -> repelled.",
      trap: "The answer changes depending on whether the rod is plastic/insulating or conducting.",
      variableMap: "End A = contact/near end\nEnd B = far end\nNegative ball = adds electrons\nConductor = charges move freely\nInsulator = charges stay local",
    });
  }

  // ------------------------------------------------------------
  // Coulomb vector components on q0 at origin
  // ------------------------------------------------------------
  if (hasAny(["q_0", "q subscript 0", "particle 0", "i with caret", "j with caret", "k with caret", "d_1", "d subscript 1"])) {
    if (hasAny(["particle 3", "q_3", "q subscript 3", "(0, d_2, d_2)", "d 2, d 2"])) {
      return makeElectroResult({
        topic: "Ch. 23/24 HW: Coulomb Force Vector from Particle 3",
        unknown: "Force components on particle 0 due only to particle 3",
        formula: "F = kq0q3/r^2, then split into components",
        math:
          "Particle 0 is at origin.\n" +
          ...
          "Magnitude:\n" +
          "F = kq0q3/(2d2^2)\n\n" +
          "Because q0 and q3 are positive, particle 3 repels particle 0.\n" +
          "Direction is negative j and negative k, equally split at 45 degrees.",
        solution:
          "i component: 0\n" +
          "j component: -k*q_0*q_3/(2*sqrt(2)*d_2^2)\n" +
          "k component: -k*q_0*q_3/(2*sqrt(2)*d_2^2)",
        trap: "Do not forget the extra sqrt(2) from splitting the force into equal y and z components.",
        variableMap: "q0 = charge at origin\nq3 = positive charge at (0,d2,d2)\nr = sqrt(2)d2",
      });
    }

    if (hasAny(["ratio d", "d_1 divided by d_2", "no net force", "balance"])) {
      return makeElectroResult({
        topic: "Ch. 23/24 HW: Balance Coulomb Forces",
        unknown: "Ratio d1/d2 for zero net force",
        ...
        math:
          ...
          "d1^2/d2^2 = q1/q2\n\n" +
          "Take square root.",
        solution: "d1/d2 = sqrt(q_1/q_2)",
        trap: "The source charge q0 and Coulomb constant k cancel out.",
        variableMap: "q1 = positive charge causing repulsion\nq2 = magnitude of negative charge causing attraction",
      });
    }

    if (hasAny(["third", "negative q", "particle 2", "q_2", "q subscript 2"])) {
      return makeElectroResult({
        topic: "Ch. 23/24 HW: Net Coulomb Force from Two y-axis Charges",
        unknown: "i, j, k components of net force on particle 0",
        formula: "Superposition: Fnet = F1 + F2",
        math:
          "Particle 1 is positive above q0 -> repels q0 downward: negative j.\n" +
          "Particle 2 is negative above q0 -> attracts q0 upward: positive j.\n\n" +
          "Add the y-components.",
        solution:
          "i component: 0\n" +
          "j component: -k*q_0*q_1/d_1^2 + k*q_0*q_2/d_2^2\n" +
          "k component: 0",
        trap: "Use q2 as the positive magnitude of the negative charge -q2. Direction supplies the sign.",
        variableMap: "q0 = charge at origin\nq1 = positive charge at y=d1\n-q2 = negative charge at y=d2",
      });
    }

    return makeElectroResult({
      topic: "Ch. 23/24 HW: Coulomb Force Vector on q0",
      unknown: "i, j, k force components",
      formula: "F = kq0q1/r^2 with direction from attraction/repulsion",
      math:
        "Particle 0 is at the origin.\n" +
        "Particle 1 is positive on +y.\n" +
        "Both charges positive, so particle 1 repels particle 0 downward along -j.",
      solution:
        "i component: 0\n" +
        "j component: -k*q_0*q_1/d_1^2\n" +
        "k component: 0",
      trap: "The force on particle 0 points away from particle 1 because both charges are positive.",
      variableMap: "q0 = charge at origin\nq1 = positive charge at y=d1\nd1 = separation distance",
    });
  }

  // ------------------------------------------------------------
  // Coulomb force and acceleration of two charged masses
  // ------------------------------------------------------------
  if (hasAny(["kg masses", "frictionless table", "each has", "μc of charge", "uc of charge"])) {
    const mass = simpleNumberBefore("kg");
    const distance = simpleNumberBefore("m\\b") || 1.0;
    const microC = simpleNumberBefore("(?:μc|uc|microc|micro c)");

    let solution = "Use F = kq^2/r^2, then a = F/m.";
    if (mass && microC && distance) {
      const q = microC * 1e-6;
      const F = K * q * q / (distance * distance);
      const a = F / mass;
      solution =
        `q = ${microC} μC = ${q.toExponential(3)} C\n` +
        `r = ${distance} m\n\n` +
        `F = kq^2/r^2\n` +
        `F = ${nice(F)} N\n\n` +
        `a = F/m\n` +
    ...
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Coulomb Force and Acceleration",
      unknown: "Electric force and/or initial acceleration",
      formula: "F = kq^2/r^2 and a = F/m",
      math: "First calculate electric force. Then plug force into Newton's Second Law.",
      solution,
      trap: "Convert μC to C. The mass does not affect electric force, only acceleration.",
      variableMap: "q = charge on each mass\nr = distance\nm = mass\nF = electric force\na = acceleration",
    });
  }

  // ------------------------------------------------------------
  // Electric force from uniform field / drag balance / soot
  // ------------------------------------------------------------
  if (hasAny(["electrostatic precipitator", "drag force", "constant speed", "soot", "charged particle"])) {
    const Efield = sciValue([/([-+]?\d*\.?\d+)\s*e\s*([-+]?\d+)\s*n\/c/]) || simpleNumberBefore("n/c");
    const drag = sciValue([/drag force.*?([-+]?\d*\.?\d+)\s*e\s*([-+]?\d+)/]) || null;

    return makeElectroResult({
      topic: "Ch. 23/24 HW: Electric Field Force Balanced by Drag",
      unknown: "Particle charge q or field from isolated particle",
      formula: "F_E = qE and E_point = k|q|/r^2",
      math:
        "If the soot particle moves at constant speed, net force is zero.\n" +
        "So electric force balances drag:\n" +
        "qE = F_drag\n" +
        "q = F_drag/E\n\n" +
        "If isolated and asked for its field at distance r:\n" +
        "E = k|q|/r^2",
      solution:
        "For your homework numbers:\n" +
        "q = (7.25 x 10^-11 N)/(1.00 x 10^5 N/C)\n" +
        "q = 7.25 x 10^-16 C\n\n" +
        "At r = 1.00 m:\n" +
        "E = kq/r^2\n" +
        "E = 6.52 x 10^-6 N/C",
      trap: "Constant speed means zero net force, not zero electric force.",
      variableMap: "E = applied electric field\nF_drag = drag force\nq = particle charge\nr = distance from isolated particle",
    });
  }
  ...
  if (hasAny(["electric field", "plastic bead", "charged to", "nc", "cm from"])) {
    const chargeNc = simpleNumberBefore("nc");
    const cm = simpleNumberBefore("cm");
    let solution = "Use E = k|q|/r^2. Direction is away from positive charge and toward negative charge.";

    if (chargeNc && cm) {
      const q = Math.abs(chargeNc) * 1e-9;
      const r = cm / 100;
      const E = K * q / (r * r);
      const direction = lower.includes("-") || lower.includes("minus") ? "toward the bead" : "away from the bead";
      solution =
        `q = ${chargeNc} nC = ${q.toExponential(3)} C\n` +
        `r = ${cm} cm = ${r} m\n\n` +
        `E = k|q|/r^2\n` +
    ...
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Electric Field of a Point Charge",
      unknown: "Electric field magnitude and/or direction",
      formula: "E = k|q|/r^2",
      math: "Convert nC to C and cm to m. Use magnitude for strength.",
      solution,
      trap: "Field points away from positive charges and toward negative charges.",
      variableMap: "q = source charge\nr = distance\nE = electric field strength",
    });
  }

  // ------------------------------------------------------------
  // PhET electric field / dipole reasoning
  // ------------------------------------------------------------
  if (hasAny(["e-field sensor", "positive charge", "negative charge", "dipole", "field strength", "grid"])) {
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Electric Field Simulation and Dipoles",
      unknown: "Direction, relative field strength, or zero-field location",
      formula: "Point charge: E = k|q|/r^2. Dipole far field drops faster, about 1/r^3.",
      math:
        "Single positive charge:\n" +
        "Field points radially away.\n\n" +
        "Single negative charge:\n" +
        "Field points radially inward.\n\n" +
        "Same distance from same charge:\n" +
        "Same magnitude in every direction.\n\n" +
        "Doubling distance from a point charge:\n" +
        "Field becomes 1/4 as large.\n\n" +
        "Two equal positive charges:\n" +
        "Field is roughly zero at midpoint.\n\n" +
        "Dipole midpoint, positive on left and negative on right:\n" +
        "Field points right.\n\n" +
        "Small dipole far away:\n" +
        "Field decreases faster than 1/r^2.",
      solution:
        "Common answers from your homework:\n" +
        "Positive charge field: radially away.\n" +
        "Negative charge field: radially inward, same strength if |q| same.\n" +
        "At 1 m vs 2 m: 1 m field is four times stronger.\n" +
        "If E = 9 V/m at 1 m, then at 3 m E = 1 V/m.\n" +
        "Two positives: midpoint field is roughly zero.\n" +
        "0.5 m above midpoint with each field 18 V/m at 45°: total ≈ 25 V/m.\n" +
        "Dipole midpoint: field points from + toward -, so to the right.\n" +
        "Small dipole: decreases more quickly than 1/r^2.",
      trap: "Electric field is a vector. Add components, not just magnitudes.",
      variableMap: "E = electric field\nr = distance\npositive source -> field away\nnegative source -> field inward",
    });
  }
  ...
  if (hasAny(["finite charged wire", "wire of length", "linear charge density", "point p", "midpoint of the wire"])) {
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Electric Field of a Finite Charged Wire",
      unknown: "Direction and magnitude of E at point P",
      formula: "E = 2*k*lambda*L / (d*sqrt(d^2 + L^2))",
      math:
        "Wire lies on x-axis from -L to +L.\n" +
        "Point P is on +y-axis at distance d.\n\n" +
        "By symmetry:\n" +
        "x-components cancel.\n" +
        "z-components are zero.\n" +
        "y-components add.\n\n" +
        "For a small piece dq = lambda dx:\n" +
        "dE_y = k*lambda*d*dx/(x^2+d^2)^(3/2)\n\n" +
      ...
      solution:
        ...
        "Mastering entry:\n" +
        "2*k*lambda*L/(d*sqrt(d^2+L^2))",
      trap: "Do not include x-components. Symmetry cancels them.",
  ...
  if (hasAny(["uniformly charged ring", "ring in the xy", "z axis", "radius a"])) {
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Electric Field of a Charged Ring",
      unknown: "Direction, field magnitude, trajectory, or angular frequency",
      formula: "E(z) = k*q*z/(z^2+a^2)^(3/2)",
      math:
        "For a uniformly charged ring in the xy-plane:\n" +
        "All x/y components cancel by symmetry.\n" +
        "Only the z-component survives.\n\n" +
        "Distance from every charge element to point z:\n" +
        "r = sqrt(z^2 + a^2)\n\n" +
        "Field along z-axis:\n" +
        "E(z) = k*q*z/(z^2+a^2)^(3/2)\n\n" +
        "Near the center, z << a:\n" +
        "E(z) ≈ k*q*z/a^3\n\n" +
        "For a negative charge -q0:\n" +
        "F = -q0E = -(k*q*q0/a^3)z\n" +
      ...
      solution:
        ...
        "Magnitude:\n" +
        "E(z) = k*q*z/(z^2+a^2)^(3/2)\n\n" +
        "If Mastering rejects exponent format, try:\n" +
        "k*q*z/((z^2+a^2)*sqrt(z^2+a^2))\n\n" +
        "Trajectory of negative ball near center:\n" +
        "oscillating along z axis between z=d and z=-d.\n\n" +
        "Angular frequency:\n" +
        "omega = sqrt(k*q*q_0/(m*a^3))",
      trap: "Mastering may reject fractional exponents. Use ((z^2+a^2)*sqrt(z^2+a^2)) as the denominator.",
  ...
  if (hasAny(["parallel-plate capacitor", "diameter electrodes", "electric field strength inside", "charge on each electrode"])) {
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Parallel-Plate Capacitor Charge from Electric Field",
      unknown: "Charge Q on each electrode",
      formula: "E = Q/(epsilon0*A), so Q = epsilon0*A*E",
      math:
        "Convert diameter to radius.\n" +
        "Find circular plate area:\n" +
        "A = pi*r^2\n\n" +
        "Use parallel-plate field:\n" +
        "E = Q/(epsilon0*A)\n" +
        "Q = epsilon0*A*E",
      solution:
        "For your homework numbers:\n" +
        ...
        "Q = 1.50 x 10^-7 C\n" +
        "Q = 150 nC",
      trap: "Plate spacing is not needed for Q if E is already given.",
      variableMap: "E = field inside capacitor\nA = plate area\nepsilon0 = 8.85 x 10^-12\nQ = charge on each plate",
    });
  }
  ...
  if (hasAny(["proton", "oppositely charged parallel plates", "released from rest", "strikes", "time interval"])) {
    return makeElectroResult({
      topic: "Ch. 23/24 HW: Proton Accelerating Between Plates",
      unknown: "Electric field E or final speed v",
      formula: "d = 1/2 at^2, qE = ma, v = at",
      math:
        "The proton starts from rest and accelerates uniformly.\n\n" +
        "First find acceleration:\n" +
        "d = 1/2 a t^2\n" +
        "a = 2d/t^2\n\n" +
        "Then use electric force:\n" +
        "qE = ma\n" +
        "E = ma/q\n\n" +
        "Final speed:\n" +
        "v = at",
      solution:
        ...
        "v = at\n" +
        "v = 2.05 x 10^4 m/s",
      trap: "The correct electric field is 147 N/C, not 9.17e3 N/C. Watch powers of ten when dividing by proton charge.",
      variableMap: "d = plate separation\nt = travel time\nm_p = proton mass\nq_p = proton charge\na = acceleration\nE = electric field",
    });
  }
