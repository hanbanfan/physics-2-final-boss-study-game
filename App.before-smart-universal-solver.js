...
function solveHomework(text, testMode = false) {
  ...
  if (
    hasAny(lower, ["proton", "released from rest"]) &&
    hasAny(lower, ["electric field", "field strength", "n/c"]) &&
    hasAny(lower, ["parallel-plate", "parallel plate", "capacitor", "spacing", "negative plate"])
  ) {
    const fieldSci = sci[0];
    const fieldPlain = firstNum("n\\s*/\\s*c");
    const E = fieldSci || fieldPlain;

    let d = null;
    const mm = firstNum("mm");
    const cm = firstNum("cm");
    const meters = firstNum("m\\b");
    ...
    if (!(E && d)) {
      return unknownFallback(
        "Ch. 23/24 — Proton Speed Across Parallel Plates",
        "q*E*d = 1/2*m*v^2, so v = sqrt(2*q*E*d/m)",
        "electric field E and plate spacing d",
        "Convert mm to meters before solving.",
        "Electric field gives energy to the proton."
      );
    }

    const q = PROTON_CHARGE;
    const m = PROTON_MASS;
    const v = Math.sqrt((2 * q * E * d) / m);

    return resultPlus({
      topic: "Ch. 23/24 — Proton Speed Across Parallel Plates",
      confidence: "high confidence",
      givens,
      unknown: "Proton speed v at the negative plate",
      formula: "q*E*d = 1/2*m*v^2, so v = sqrt(2*q*E*d/m)",
      steps:
        "The proton starts from rest, so electric potential energy becomes kinetic energy.\\n" +
        "Work by electric field = q*E*d.\\n" +
        "Kinetic energy gained = 1/2*m*v^2.\\n" +
        "Set q*E*d = 1/2*m*v^2.\\n" +
        `E = ${E.toExponential(3)} N/C\\n` +
        `d = ${d.toExponential(3)} m\\n` +
        "q_proton = 1.60 x 10^-19 C\\n" +
        "m_proton = 1.67 x 10^-27 kg",
      answer:
        `v = sqrt((2)(1.60 x 10^-19)(${E.toExponential(3)})(${d.toExponential(3)})/(1.67 x 10^-27))\\n\\n` +
        `Final Answer: v = ${v.toExponential(3)} m/s`,
      trap:
        "The plate spacing must be in meters. 1.90 mm = 1.90 x 10^-3 m.",
      memory:
        "Released from rest across plates = electric work becomes kinetic energy.",
    });
  }
