...
function findStepBody(steps, searchWords) {
  const found = steps.find((step) =>
    searchWords.some((word) =>
      step.title.toLowerCase().includes(word.toLowerCase())
    )
  );
  return found ? found.body : "";
}

function buildGivenMathSolution(problemAnalysis) {
  const steps = problemAnalysis.steps || [];

  const givens = findStepBody(steps, ["givens", "given"]);
  const unknown = findStepBody(steps, ["unknown"]);
  const formula = findStepBody(steps, ["formula"]);
  const rearrange = findStepBody(steps, ["rearrange"]);
  const substitute = findStepBody(steps, ["substitute"]);
  const calculate = findStepBody(steps, ["calculate"]);
  const boxed = findStepBody(steps, ["box", "answer"]);
  const unitCheck = findStepBody(steps, ["unit"]);
  const fullWork = findStepBody(steps, ["full work"]);

  return {
    given:
      `Problem Type: ${problemAnalysis.topic || "Unknown"}\n\n` +
      `Clue Words: ${
        problemAnalysis.clues && problemAnalysis.clues.length
          ? problemAnalysis.clues.join(" | ")
          : "No clue words found yet."
      }\n\n` +
      `Givens: ${givens || "No givens found yet."}\n\n` +
      `Unknown: ${unknown || problemAnalysis.unknown || "Unknown"}`,

    math:
      `Formula:\n${formula || problemAnalysis.formula || "No formula yet."}\n\n` +
      `Rearrange:\n${rearrange || "Solve the formula for the unknown first."}\n\n` +
      `Substitute:\n${substitute || "Plug numbers into the rearranged formula."}\n\n` +
      `Calculate:\n${calculate || "Do the arithmetic one line at a time."}`,

    solution:
      `Final Answer:\n${problemAnalysis.finalAnswer || boxed || "No final answer yet."}\n\n` +
      `Unit Check:\n${unitCheck || "Make sure the unit matches the unknown."}\n\n` +
      `Copyable Work:\n${fullWork || boxed || "Once enough information is entered, the full work will appear here."}`,
  };
}
...
export default function App() {
  ...
  if (screen === "smartSolver") {
    const currentProblemStep =
      problemAnalysis.steps[Math.min(problemStepIndex, problemAnalysis.steps.length - 1)];

    const gms = buildGivenMathSolution(problemAnalysis);

    return (
      <Screen calm={calmMode}>
        <Header title="Homework Problem Solver" subtitle="GIVEN → MATH → SOLUTION" />

        <Card>
          <Text style={styles.bigText}>Paste one homework problem.</Text>
          <Text style={styles.body}>
            The app will organize the problem into GIVEN, MATH, and SOLUTION, then walk you through each step.
          </Text>
          ...
          <TextInput
            ...
            onChangeText={(text) => {
              ...
              setProblemStepIndex(0);
            }}
            placeholder="Paste the full problem here with numbers and units."
            placeholderTextColor="#64748b"
            multiline
          />

          <View style={styles.gmsBox}>
            <Text style={styles.gmsHeader}>GIVEN</Text>
            <Text style={styles.gmsText}>{gms.given}</Text>
          </View>

          <View style={styles.gmsBox}>
            <Text style={styles.gmsHeader}>MATH</Text>
            <Text style={styles.gmsText}>{gms.math}</Text>
          </View>

          <View style={styles.solutionBox}>
            <Text style={styles.solutionHeader}>SOLUTION</Text>
            <Text style={styles.solutionText}>{gms.solution}</Text>
          </View>

          <Text style={styles.miniHeader}>Step-by-Step Tutor Mode</Text>

          <View style={styles.solverStepCard}>
            <Text style={styles.solverStepTitle}>
              {currentProblemStep?.title || "Step"}
            </Text>
            <Text style={styles.solverStepBody}>
              {currentProblemStep?.body || "Paste a problem first."}
            </Text>
          </View>
          ...
          <View style={styles.videoControls}>
            <Pressable
              ...
              onPress={() => setProblemStepIndex(Math.max(problemStepIndex - 1, 0))}
            >
            ...
            <Pressable
              ...
              onPress={() =>
                setProblemStepIndex(
                  problemStepIndex + 1 < problemAnalysis.steps.length
                    ? problemStepIndex + 1
                    : 0
                )
              }
            >
              <Text style={styles.playText}>
                {problemStepIndex + 1 < problemAnalysis.steps.length ? "Next Step" : "Restart"}
              </Text>
            </Pressable>
          </View>

          <Text style={styles.miniHeader}>All Steps</Text>
          {problemAnalysis.steps.map((step, index) => (
            <View key={`${step.title}-${index}`} style={styles.allStepCard}>
              <Text style={styles.allStepTitle}>{step.title}</Text>
              <Text style={styles.allStepBody}>{step.body}</Text>
            </View>
          ))}
