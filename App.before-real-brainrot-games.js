...
/*
  ...
  Structure:
  Home → Module → Chapter → Study Guide / Clips / Flashcards / Formula Map / Brainrot Game / Solver
...
export default function App() {
  ...
  const gameCard = chapter.brainrot[gameIndex % chapter.brainrot.length];
  ...
  function gameChoices() {
    const wrongs = chapter.flashcards.map((item) => item[1]).filter((item) => item !== gameCard.a);
    const choices = [gameCard.a, wrongs[gameIndex % wrongs.length], wrongs[(gameIndex + 3) % wrongs.length], wrongs[(gameIndex + 7) % wrongs.length]];
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          <Box title="Module Mission">
            {`${module.dates}\n${module.exam}\n\nPick a chapter below, then choose Study Guide, Flashcards, Clips, Formula Map, Brainrot Game, or Solver.`}
          </Box>
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Button label="Brainrot Chapter Game" type="gold" onPress={() => { setGameIndex(0); setGameScore(0); setGameMessage(""); setScreen("game"); }} />
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.card}>
          ...
          <Button label="Flashcards" type="purple" onPress={() => { setFlashIndex(0); setShowAnswer(false); setScreen("flashcards"); }} />
          <Button label="Brainrot Game" type="gold" onPress={() => { setGameIndex(0); setGameScore(0); setGameMessage(""); setScreen("game"); }} />
  ...
  if (screen === "game") {
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>Brainrot Chapter Game</Text>
        <Text style={styles.subtitle}>Chapter {chapter.number}: {chapter.title}</Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.gameQuestionBox}>
            <Text style={styles.gameLabel}>BRAINROT CHECK {gameIndex + 1}</Text>
            <Text style={styles.gameQuestion}>{gameCard.q}</Text>
          </View>
          ...
          {choices.map((choice) => (
            <Pressable
              ...
              onPress={() => {
                if (choice === gameCard.a) {
                  setGameScore((old) => old + 10);
                  setGameMessage(`Correct. ${gameCard.why}`);
                } else {
                  setGameMessage(`Not quite. Correct: ${gameCard.a}. ${gameCard.why}`);
                }
              }}
            >
          ...
          <Button label="Next Question" type="gold" onPress={() => { setGameIndex(gameIndex + 1 < chapter.brainrot.length ? gameIndex + 1 : 0); setGameMessage(""); }} />
          <Button label="Back to Chapter" type="secondary" onPress={() => setScreen("chapter")} />
        </View>
      </ScrollView>
    );
